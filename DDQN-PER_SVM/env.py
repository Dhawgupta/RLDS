...
class DialougeSimulation(object):
    ...
    def step(self, action):
        ...
        new_state = np.array([float('%.3f' % elem) for elem in self.current_state])
        # if action == 0 and self.num_iter == 0:  # greeting should be first
        # new_state[0] = 1
        if action >= 0 and action <= 4:  # the action is to ask a slot value
            new_state[action] = 0.4 * (random.random() * (-2) + 1) + 0.60  # the confidenec value lies between 0.6 - 0.8
        if action > 6 and action <= 11:
            ...
            if new_state[action - 7] < 0.1:  # @1/3/18 18:49:00this implies that the ask part has not happened
                ...
                new_state[action - 7] = random.random() * 0.2 + 0.8  # @1/3/18 18:49:00the ask part has happned
        if action == 5:
            new_state[0] = 0.4 * (random.random() * (-2) + 1) + 0.60  # 0.2 * random.random() + 0.55
            new_state[1] = 0.4 * (random.random() * (-2) + 1) + 0.60  # 0.2 * random.random() + 0.55
        if action == 6:
            new_state[2] = 0.4 * (random.random() * (-2) + 1) + 0.60  # 0.2 * random.random() + 0.55
            new_state[3] = 0.4 * (random.random() * (-2) + 1) + 0.60  # 0.2 * random.random() + 0.55
